...
class MediaAdvisor(KnowledgeEngine):
    ...
    def advise(self, environment, job, feedback, student_profile=None):
        ...
        Args:
            environment: Learning environment (str)
            job: Job type (str)
            feedback: Feedback requirement (str)
    ...
    def _display_summary(self):
        ...
        print("=" * 70)
        print(f"⏱️  Execution time: {self.execution_time:.3f} seconds")
    ...
    def _get_final_recommendation(self):
        ...
        if not media_facts:
            print("⚠️  No recommendation was found!")
            return None
        ...
        print(f"⚠️  Note: {len(media_facts)} recommendations found")
